...
export const MenuItemFetch = async () => {
  const res = await axios.get<MenuItemApiResponse>(
    `${process.env.NEXT_PUBLIC_STRAPI_IP_DEV}/api/menu-items?populate[0]=highlight.img&populate[1]=highlight.links`
  );
